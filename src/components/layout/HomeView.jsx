import { Card, Image, Group, Text, Title, Grid } from '@mantine/core';
import { Link } from 'react-router-dom';

export default function HomeView() {
  return (
    <div style={{ padding: '2rem' }}>
      <Grid gutter="xl" align="stretch">
  <Grid.Col span={{ base: 12, md: 6 }}>

        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Card.Section style={{ height: 550, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  <Image
    src="/assets/home-network-designer.png"
    alt="Network Designer"
    height={200}
    fit="contain"
    style={{ maxWidth: '100%', height: '100%', objectFit: 'contain' }}
  />
</Card.Section>
          <Group position="apart" mt="md" mb="xs">
            <Text weight={400}>Network Designer</Text>
          </Group>
          <Text size="sm" color="dimmed">
            Plan and visualize your IP subnets interactively. Allocate, manage, and export network diagrams.
          </Text>
          <Link to="/network-designer" style={{ textDecoration: 'none' }}>
            <Text mt="md" color="blue" weight={700}>
              Open Network Designer →
            </Text>
          </Link>
        </Card>
  </Grid.Col>
  <Grid.Col span={{ base: 12, md: 6 }}>
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Card.Section style={{ height: 550, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  <Image
    src="/assets/home-azure-naming.png"
    alt="Azure Naming Tool"
    height={200}
    fit="contain"
    style={{ maxWidth: '100%', height: '100%', objectFit: 'contain' }}
  />
</Card.Section>
          <Group position="apart" mt="md" mb="xs">
            <Text weight={400}>Azure Naming Tool</Text>
          </Group>
          <Text size="sm" color="dimmed">
            Generate consistent Azure resource names following best practices and your conventions.
          </Text>
          <Link to="/azure-naming" style={{ textDecoration: 'none' }}>
            <Text mt="md" color="blue" weight={700}>
              Open Azure Naming Tool →
            </Text>
          </Link>
        </Card>
  </Grid.Col>
</Grid>
    </div>
  );
}

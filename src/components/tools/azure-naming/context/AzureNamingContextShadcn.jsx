...
// Context
const AzureNamingContextShadcn = createContext();
...
export const AzureNamingProviderShadcn = ({ children }) => {
  ...
  const generateName = async () => {
    ...
    try {
      ...
      toast.success('Names Generated Successfully', {
        description: `Generated ${generatedNames.length} Azure resource name${generatedNames.length > 1 ? 's' : ''}`,
        icon: <Cloud size={16} />
      });
      ...
      toast.error('Name Generation Failed', {
        description: error.message || 'Failed to generate Azure resource names',
        icon: <X size={16} />
      });
  ...
  return (
    <AzureNamingContextShadcn.Provider value={value}>
      {children}
    </AzureNamingContextShadcn.Provider>
  );
};

// Hook
export const useAzureNamingContextShadcn = () => {
  const context = useContext(AzureNamingContextShadcn);
  if (!context) {
    throw new Error('useAzureNamingContextShadcn must be used within an AzureNamingProviderShadcn');
  }
  return context;
};